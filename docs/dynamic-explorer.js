...
class DependencyExplorer {
    ...
    async loadGraph(graphType) {
        ...
        try {
            ...
            // Show helpful message for missing large graphs
            if (graphType === 'type-deps' || graphType === 'proof-deps') {
                this.container.select('.loading').html(`
                    <div class="error">
                        <strong>${graphType} graph not available online</strong><br>
                        This graph is too large for GitHub Pages (${graphType === 'type-deps' ? '165MB' : '665MB'}).<br><br>
                        <strong>To use locally:</strong><br>
                        1. Clone the repository: <code>git clone https://github.com/aurasoph/lean-graph.git</code><br>
                        2. Generate the data: <code>cd lean-graph/web-explorer && python3 convert_to_json.py</code><br>
                        3. Start local server: <code>python3 -m http.server 8000</code><br>
                        4. Open: <code>http://localhost:8000</code>
                    </div>
                `);
            } else {
