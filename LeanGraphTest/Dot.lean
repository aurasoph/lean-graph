...
def runGraphCommand : IO Unit := do
  let _ ← IO.Process.output {
    cmd := "lake"
    args := #["exe", "graph", "--to", "LeanGraphTest.ToTarget", "--mark-sorry", "LeanGraphTest/produced.dot"]
  }
...
#eval show IO Unit from do
  runGraphCommand
  let expectedOutput ← readFile "LeanGraphTest/expected.dot"
  let actualOutput ← readFile "LeanGraphTest/produced.dot"
